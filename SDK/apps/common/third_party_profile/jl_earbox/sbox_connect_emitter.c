...
void clear_last_info(void)
{
    u8 mac_addr[6];
    printf("%s[line:%d]\n",__func__,__LINE__);
    u8 flag = bt_restore_remote_device_info_opt(mac_addr, 1, get_remote_dev_info_index());
    if(flag){
...
static void earphone_sync_sbox_setting_receive(void *_data, u16 len, bool rx)
{
    if (rx) {
        ...
        case SYNC_CMD_CHANGE_LMP_ADDR:
            extern void sync_change_lmp_addr_deal(u8 *data, u16 len);
            sync_change_lmp_addr_deal(&data[1], len-1);
            break;
        case SYNC_CMD_ADDR_CHANGE_FLAG:
            extern void sync_addr_change_flag_deal(u8 *data, u16 len);
...
extern void custom_sync_wait_emitter_conn(void);
