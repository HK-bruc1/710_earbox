...
struct s_box_app_cb {
    ...
    void (*sbox_sync_bt_con_state)(void);           //同步的经典蓝牙状态给仓
    void (*sbox_sync_ble_con_state)(void);          //同步BLE状态给仓
 ...
 * 参数：service：对应服务的handle  handle_type:ble发数的方式
 * 返回值：无
 */
extern void sbox_ble_att_send_data(u8 cmd, u8 *data, u16 len);
