...
{
    r_printf("%s", __func__);
    handle_send_packect(key_power, sizeof(key_power), REPROT_CONSUMER_LEN);//电源键都ok
}

void key_memu_ctrl(void)
{
    r_printf("%s", __func__);
    handle_send_packect(key_nemu, sizeof(key_nemu), REPROT_CONSUMER_LEN);//ios菜单ok  安卓挂
    handle_send_packect(key_android_home, sizeof(key_android_home), REPROT_CONSUMER_LEN);
}
...
int bt_connect_reset_xy_vm(void);
...
#endif
    if (REMOTE_IS_IOS()) {
        handle_send_packect(key_reset_coordinate, sizeof(key_reset_coordinate), REPROT_INFO_LEN0);
    }
}
...
static void key_send_coordinate(s16 axis_x,s16 axis_y)
{
    ...
    if (REMOTE_IS_IOS()) {
#if TCFG_USER_EDR_ENABLE
        edr_hid_data_send(5, second_packet.data, sizeof(second_packet.data));
#endif
    }
}
// 抖音坐标右移
...
static u8 *move_str[4] = {"up", "down", "left", "right"};
...
static void keypage_coordinate_equal_ios(MOVE_DIRECTION dir)
{
    ...
    if (REMOTE_IS_IOS()) {
        handle_send_packect(key_focus_ios_reset, sizeof(key_focus_ios_reset), REPROT_INFO_LEN0);
    }
#else
    u16 ios_x = 0,ios_y = 0;
...
static void keypage_coordinate_equal(MOVE_DIRECTION dir)
{
    ...
    handle_send_packect(key_focus_android, sizeof(key_focus_android), REPROT_INFO_LEN1);
}
...
void keypage_photo(void)
{
    ...
    if (REMOTE_IS_IOS()) {
        ...
        handle_send_packect(key_photo_android, sizeof(key_photo_android), REPROT_INFO_LEN1);
    }
}

//自动定时循环, 点击已定位的坐标点
static void keypage_auto_click_focus_loop(void)
{
    log_info("%s", __func__);
    if (REMOTE_IS_IOS()) {
        handle_send_packect(key_pp_release_before, sizeof(key_pp_release_before), REPROT_INFO_LEN0);
...
static void keypage_auto_next_loop(void)
{
    log_info("%s", __func__);
    if (REMOTE_IS_IOS()) {
 ...
 void keypage_app_key_deal_test(u8 key_value)
{
    ...
    switch (key_value) {
        case 0:
            if (REMOTE_IS_IOS()) {
                handle_send_packect(key_up_before, sizeof(key_up_before), REPROT_INFO_LEN0);    //上滑
            }else
        ...
        case 1:
            if (REMOTE_IS_IOS()) {
                handle_send_packect(key_down_before, sizeof(key_down_before), REPROT_INFO_LEN0); //下滑
            }else
            handle_send_packect(key_down_click, sizeof(key_down_click), REPROT_INFO_LEN1);
            break;
        ...
        case 2:
            if (REMOTE_IS_IOS()) {
                handle_send_packect(key_left_before, sizeof(key_left_before), REPROT_INFO_LEN0);  //左滑
            }else 
        ...
        case 3:
            if (REMOTE_IS_IOS()) {
                handle_send_packect(key_right_before, sizeof(key_right_before), REPROT_INFO_LEN0);   //右滑
            }else
        ...
        case 4: {
            ...
            while (count--) {
                if (REMOTE_IS_IOS()) {
                    handle_send_packect(key_pp_release_before, sizeof(key_pp_release_before), REPROT_INFO_LEN0);   //点赞
                }else{
