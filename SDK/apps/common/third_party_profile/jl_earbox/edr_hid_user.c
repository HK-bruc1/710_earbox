 ...
 */
/*************************************************************************************************/
extern void usr_set_remote_type(u8 flag );
extern void sync_remote_type(void);
void sdp_callback_remote_type(u8 remote_type)
{
    printf("edr_hid:remote_type= %d\n", remote_type);
    ...
    if(remote_type == 2){
        ...
        buf = usr_get_keypage_report_map(0,&len);
        if(buf){
...
void user_hid_set_icon(u32 class_type)
{
    ...
    bt_change_hci_class_type(class_type);
}
...
int user_hid_send_data(u8 *buf, u32 len)
{
    ...
    ret = bt_cmd_prepare(USER_CTRL_HID_SEND_DATA, sizeof(hid_s_param_t), (u8 *)&s_par);
...
{
    printf("func:%s , hid_channel:%d\n" , __func__ , hid_channel);
    return hid_channel;
}
...
#include "app_main.h"
#define TWS_FUNC_ID_SYNC_HID    TWS_FUNC_ID('S', 'B', 'O', 'X')

void sync_hid_channel_id_deal(u8 *data, u16 len);
#define REMOTE_TYPE_ID      0x10
#define CHANNEL_TYPE_ID     0x11
static void sync_hid_setting_receive(void *_data, u16 len, bool rx)
{
    if (rx) {
        u8 *data = (u8 *)_data;
        log_info("%s[line:%d]\n",__func__,__LINE__);
        ...
        case CHANNEL_TYPE_ID:
            sync_hid_channel_id_deal(data+1,len-1);
            break;
        default:
            break;
        }
...
REGISTER_TWS_FUNC_STUB(sync_hid_setting) = {
    .func_id = TWS_FUNC_ID_SYNC_HID,
    .func    = sync_hid_setting_receive,
};

void sync_remote_type(void)
{
    u8 msg[2];
    msg[0] = REMOTE_TYPE_ID;
    msg[1] = usr_get_remote_type();
    printf("%s, connect_remote_type:%d, \n", __func__, msg[1]);
    if(tws_api_get_tws_state() & TWS_STA_SIBLING_CONNECTED){
        tws_api_send_data_to_sibling(msg, sizeof(msg), TWS_FUNC_ID_SYNC_HID);
    }
}
...
{
    log_info("%s[line:%d]\n",__func__,__LINE__);
    hid_ctrl_channel = little_endian_read_16(data, 0); //ctrl_channel
    hid_channel = little_endian_read_16(data, 2); //inter_channel
    bt_send_busy = 0;
    log_info("tws hid connect ########################,%d,%d\n", hid_ctrl_channel, hid_channel);
    cbuf_init(&user_send_cbuf, hid_tmp_buffer, HID_TMP_BUFSIZE);
}
...
void sync_hid_channel_id(void)
{
    log_info("%s[line:%d]\n",__func__,__LINE__);
    u8 sny_data[5];
    sny_data[0] = CHANNEL_TYPE_ID;
    if(!hid_channel){
        log_info("%s[line:%d, hid channel = 0]\n",__func__,__LINE__);
        
        return;
    }
    memcpy(sny_data+1, &hid_ctrl_channel, 2);
    memcpy(&sny_data[3], &hid_channel, 2);
    if(tws_api_get_tws_state() & TWS_STA_SIBLING_CONNECTED){
        tws_api_send_data_to_sibling(sny_data, sizeof(sny_data), TWS_FUNC_ID_SYNC_HID);
    }
    // earphone_sync_sbox_setting(SYNC_CMD_HID_CHANNEL_ID, sny_data, sizeof(sny_data));
}
 ...
 * tws事件状态处理函数
 */
static int hid_tws_connction_status_event_handler(int *msg)
{
...
/*
在蓝牙配置里面初始化，在earphone.c
#if ((TCFG_BT_SUPPORT_HID==1) && TCFG_USER_EDR_ENABLE)
   edr_hid_config_init();
#endif
*/
void edr_hid_config_init(void)
{
    int len = 0;
    u8*buf= NULL;
    printf("func:%s ,usr_get_keypage_report_map \n",__func__);
