...
extern char SC7A20_click_status(void);
extern void SC7A20_int_io_detect(u8 int_io_status);
extern char SC7A20_Click_Read(int TH1, int TH2);

extern u32 SL_MEMS_i2cRead(u8 addr, u8 reg, u8 len, u8 *buf);
//带1个参数的函数
static void sc7a20_callback_func_1_param(int mode)
{
    //axis_info_t accel_data    [32];
void *arg;
    printf("/////////////////////////////%s %d\n", __func__, mode);
    sys_s_hi_timer_add(NULL, SC7A20_read_data, 100); //10ms
}
//推送某个函数到指定任务执行
 void sc7a20_func_callback_in_task(u8 mode){
    int err;
    int msg[5];
    printf("%s %d\n", __func__, err);
  
    //推送带1个参数的函数到app_core任务
    msg[0] = (int) sc7a20_callback_func_1_param;
    msg[1] = 1;//函数需要1个参数
...
int gravity_sensor_init(void *_data)
{
    ...
    printf("gSensor_info->iic_hdl===%d",gSensor_info->iic_hdl);
...
//            spin_lock_init(&iic_lock);
        ...
        // sc7a20_func_callback_in_task(1);
    }
    return 0;
}
