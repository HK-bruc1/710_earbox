...
#endif/*ANC_MULT_ORDER_ENABLE*/
    u8 adap_now_ref_lvl;			/*场景自适应FF当前等级*/
    u8 adap_now_err_lvl;			/*场景自适应FB当前等级*/
        ...
        if (res == OS_TASKQ) {
            ...
            case ANC_MSG_RUN:
                ...
                if (anc_ear_adaptive_busy_get()) {
                    anc_ear_adaptive_busy_set(0);	//耳道自适应训练结束
                }
#endif
...
//初始化ANC ADC相关参数，注意需在audio_adc_init后调用
    ...
    anc_hdl->adap_now_ref_lvl = 0xff;
    anc_hdl->adap_now_err_lvl = 0xff;
#endif/*ANC_ADAPTIVE_EN*/
...
static void anc_timer_deal(void *priv)
{
    ...
    u8 mic0_pre_gain = (JL_ADDA->ADA_CON1 >> 12) & 0x3;
    u8 mic1_pre_gain = (JL_ADDA->ADA_CON2 >> 12) & 0x3;
    u8 mic2_pre_gain = (JL_ADDA->ADA_CON3 >> 12) & 0x3;
    u8 mic3_pre_gain = (JL_ADDA->ADA_CON4 >> 12) & 0x3;
    u8 mic4_pre_gain = (JL_ADDA->ADA_CON5 >> 12) & 0x3;

    user_anc_log("MIC_G:%d,%d,%d,%d,%d, MIC_PRE:%d,%d,%d,%d,%d, DAC_AG:%d,%d,DAC_DG:%d,%d\n", \
                 mic0_gain, mic1_gain, mic2_gain, mic3_gain, mic4_gain, \
                 mic0_pre_gain, mic1_pre_gain, mic2_pre_gain, mic3_pre_gain, mic4_pre_gain, \
...
void audio_anc_mic_management(audio_anc_t *param)
{
    ...
    for (i = 0; i < 4; i++) {
        for (int cnt = 0; cnt < AUDIO_ADC_MAX_NUM; cnt++) {
            if (param->mic_type[i] == cnt) {
                param->mic_param[cnt].en = 1;
                param->mic_param[cnt].type = i;
                if (cfg->mic_en_map & BIT(cnt)) {
                    param->mic_param[cnt].mult_flag = 1;
                }
                audio_anc_mic_mana_set_gain(param, cnt, i);
            }
        }
    }
...
void audio_anc_adap_sync(int ref_lvl, int err_lvl, int err)
{
    u8 sync_flag = 0;
    //避免相同等级重复切换
    if (anc_hdl->adap_now_ref_lvl != ref_lvl) {
        anc_hdl->adap_now_ref_lvl = ref_lvl;
        sync_flag = 1;
    }
    if (sync_flag) {
