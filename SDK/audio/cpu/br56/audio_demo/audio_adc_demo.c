...
struct adc_demo {
    ...
    s16 adc_buf[ADC_DEMO_BUFS_SIZE * 2];
    u8 linein_ch_num;
    u8 linein_ch_sel;
};
    ...
    if (mic_demo && mic_demo->adc_2_dac) {
        if (adc_hdl.channel_num >= 2) {
            ...
            if (adc_hdl.bit_width) {
                ...
                audio_dac_write(&dac_hdl, temp32, len);
            } else {
                for (i = 0; i < (len / 2); i++) {
                    switch (mic_ch_sel) {
                    case 0:
                ...
                audio_dac_write(&dac_hdl, temp16, len);
            }
